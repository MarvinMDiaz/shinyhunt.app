...
export async function getLiveAnalytics(): Promise<LiveAnalytics> {
  try {
    ...
    // Get longest running active hunt
    ...
    let longestRunningHunt: LiveAnalytics['longestRunningHunt'] = null
    ...
    // Get most active hunter today (by progress events)
    ...
    let mostActiveHunter: LiveAnalytics['mostActiveHunterToday'] = null
    if (!progressError && progressEvents && progressEvents.length > 0) {
      ...
      // Find user with most resets
      let maxResets = 0
      let topUserId = ''
      userResetCounts.forEach((count, userId) => {
        if (count > maxResets) {
          maxResets = count
          topUserId = userId
        }
    ...
    return {
      ...
      newSignupsToday: 0,
      huntsStartedToday: 0,
      longestRunningHunt: null,
      mostActiveHunter: null,
    }
  }
}
